...
function DoctorCreateDietPlan() {
  ...
  const [diett, setDiett] = useState([]);

  const filterDietdata = dietDetails.filter((item) => item.customer_id === cstId);
  console.log("filterDietdata", filterDietdata);
  
  const mappeDietData = filterDietdata.map((data) => (data.diet_detail));
  console.log("mappeDietData", mappeDietData);

  const filterSupplementddata = supplementsData.filter((item) => item.customer_id === cstId);
  console.log("filterdata", filterSupplementddata);
  
  const mappeSuppData = filterSupplementddata.map((data) => (data.diet_detail));
  console.log("mappeSuppData", mappeSuppData);

  const filterPtRdata = pointsToRememberData.filter((item) => item.customer_id === cstId);
  console.log("filterPtRdata", filterPtRdata);
  
  const mappePtrData = filterPtRdata.map((data) => (data.diet_detail));
  console.log("mappePtrData", mappePtrData);
  
  // console.log("supplementsData", supplementsData);
  // console.log("pointsToRememberData", pointsToRememberData);
  ...
  const master_data_get = async (retrievedDoctorId, retriveCustomerIdid) => {
    setcstId(retriveCustomerIdid);
    console.log("setcstId", retriveCustomerIdid);
    setShowLoader(true);
    ...
    await server_post_data(my_patients_data_single, fd)
      .then((Response) => {
        if (Response.data.error) {
          ...
          // console.log("Response.data",Response.data.message.findDoctorDietPlan)
          setDietDetails(Response.data.message.findDoctorDietPlan);
          setPointsToRememberData(
            Response.data.message.findDoctorPointsToRemember
          );
          setSupplementsData(Response.data.message.findDoctorSupplements);
  ...
  const [pdfDiet, setPdfDiet] = useState("");
  ...
  const downloadPDF = async () => {
    ...
    const pdfBase64 = await new Response(pdfBlob)
      .blob()
      .then((blob) => URL.createObjectURL(blob));

    // Save the PDF data in session storage
    sessionStorage.setItem("healthPlanPdf", pdfBase64);
  ...
  const handleSaveChangesdynamic = async (form_data, url_for_save) => {
    ...
    const pdfBase64 = await downloadPDF();
    ...
    fd_from.append("diet_Plan_Pdf", pdfBase64);
    // fd_from.append('dietPlanPdf', generatedPdfData, 'generated_pdf.pdf');
    ...
    <>
      <div className="container-xl create_diet_plan">
        ...
        <div className="container " id="pdfContent" hidden>
          <div className="container-1 " ref={pdfContentRef} id="pdfContent-1">
            <div className="div">
              <div className="div-2">
                <div className="div-3">
                  <div className="div-4">
          ...
          <div className="container-2" ref={pdfContentRef} id="pdfContent-2">
          ...
          <div className="container-3" ref={pdfContentRef} id="pdfContent-3">
                            ...
                            >
                              ...
                              <div className="select_inputDietPlan diet_items">
                                ...
                                <CreatableSelect
                                  ...
                                  maxLength={100}
                                  defaultInputValue={mappeDietData}
                                />
                                <input
                                ...
                                <div className="supplymentsSelect">
                                  <div className="select_inputDietPlan diet_items">
                                    ...
                                    <CreatableSelect
                                      ...
                                      maxLength={100}
                                      defaultInputValue={mappeSuppData}
                                    />
                                    {/* <input
                                      className="trio_mendate  height_input2 hidden"
                                      type="text"
                ...
                <div className="create_diet_patient_wrapper">
                  <div className="create_diet_patient_details_div">
                    <div className="create_diet_patient_img_div">
                      <img
                        src={`${upcomingAppointImage}${patients.customer_image}`}
                        onError={(e) => {
